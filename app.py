...
with st.sidebar.expander("📜 내 토큰 이용 내역 보기"):
    my_history = [r for r in ws_history.get_all_values()[1:] if r[1] == user_name]
    if my_history:
            ...
            with st.container():
                ...
                if st.session_state.get(unlock_key, False):
                    ...
                    if st.button(f"🔓 상세 정보 열람 (토큰 1개 차감)", key=f"btn_own_{idx}"):
                        if user_tokens >= 1:
                            update_token(-1, f"소유주 매물 열람 ({name}, {addr} {room})")
                            st.session_state[unlock_key] = True
                            st.rerun()
                        else:
                            st.error("보유한 열람권(토큰)이 부족합니다.")
